...
import com.alibaba.schedulerx.shade.com.google.common.collect.ArrayListMultimap;
import com.alibaba.schedulerx.shade.com.google.common.collect.Multimap;
...
/**
 * @author ：luke
 * @date ：Created in 2024/5/15 3:44 PM
 * @description：动态路由认证
 * @modified By：
 */
    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (userInfo instanceof UserDetails) {
            ...
            List<PermissionDO> btnPerms = permissions.stream()
                    // 过滤页面权限
                    .filter(permission -> Objects.equals(permission.getType(), ConstantPool.BUTTON))
                    // 过滤 URL 为空
                    .filter(permission -> StrUtil.isNotBlank(permission.getUrl()))
                    // 过滤 METHOD 为空
                    .filter(permission -> StrUtil.isNotBlank(permission.getMethod())).toList();

            for (PermissionDO btnPerm : btnPerms) {
